eLORD <- function(d, alpha = 0.05, gammai, w0=0.0001, rho_reward = 0.5, rho_penalty = 0.5, random = TRUE,
...
eLORD_faster <- function(eval, alpha, w0, rho_reward, rho_penalty, display_progress = TRUE) {
  N <- length(eval)
  # 初始化变量
  alphai <- numeric(N)
  wt <- numeric(N)
